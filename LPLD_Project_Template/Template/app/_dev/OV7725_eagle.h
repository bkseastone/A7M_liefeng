...
#define CAMERA_H (60u)		//图像高度 60
#define CAMERA_PAGE (1u)	//图像张数
...
typedef struct {
	volatile uint8 pic1_data[CAMERA_H][CAMERA_W/8];
#if   (CAMERA_PAGE == 2)
	uint8 occupy2[2]; 	//跳过VSYN与HSYN间的16个像素
	volatile uint8 pic2_data[CAMERA_H][CAMERA_W/8];
#elif (CAMERA_PAGE == 3)
	uint8 occupy3[2];
	volatile uint8 pic3_data[CAMERA_H][CAMERA_W/8];
#endif
} OV_pictureTypeDef_SRAM;
